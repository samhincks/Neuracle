   ...
   function getSAXData(control) {
        console.log(control);
        console.log(control.form);
        console.log("Action = " + control.form.action);
        var form = control.form; //.. retrieve the corresponding form
       
       //.. Create Prototypes AJAX.Request, passing form's inputs as parameters'
        new Ajax.Request(form.action, { 
            method: 'post',
            parameters : form.serialize(),
            onSuccess: receiveSAXResponse } //.. when we're done call function below
        );
    }
    //.. called by prototype
    function receiveSAXResponse(xhr) {
        console.log("received response");       
        var JSONobj = eval('('+ xhr.responseText +')');   
        drawLinePlotSAX(JSONobj);
    }
            ...
            for (var j = 1; j < channels.length; j++) {
                ...
                for (var i =0; i <data.length; i++) {
                    ...
                    if (point.scope != null) {
                        ...
                        console.log("setting " + colIndex + " to true");
                    }
