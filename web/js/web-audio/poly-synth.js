...
function PolySynth(context) {
 ...
 this.createNoisegateSideChainGraph_ = function (inputNode) {
     var noisegateOutputNode = new GainNode(this.context_);
  ...
  this.createDelayGraph_ = function(inputNode) {
    ...
    var delayEffectOutputNode = new GainNode(this.context_, {gain: 1});
   ...
   this.createBitcrusherGraph_ = function(inputNode) {
    var bitcrusherOutputNode = new GainNode(this.context_);
  ...
  this.createReverbGraph_ = function(inputNode) {
    ...
    var reverbOutputNode = new GainNode(this.context_);
...
this.parameters_ = {
  ...
  gainRelease: 0.13,
  filterCutoff: 440,//60, // filter is not active
  filterQ: 15.35,
  filterAttack: 0,
  filterDecay: 0.58,
...
// ADDING THIS JAN 2018
this.voiceOutput_.connect(this.output);

/*
// The output of |this.voiceOutput_| is processed by four effects:
// noisegate-sidechaining, bitcrushing, delay, and reverb.

var noisegateInputNode = new GainNode(this.context_);
var noisegateOutputNode =
    this.createNoisegateSideChainGraph_(noisegateInputNode);

var bitcrusherInputNode = new GainNode(this.context_);
var bitcrusherOutputNode = this.createBitcrusherGraph_(bitcrusherInputNode);

var delayInputNode = new GainNode(this.context_);
var delayOutputNode = this.createDelayGraph_(delayInputNode);

var reverbInputNode = new GainNode(this.context_);
var reverbOutputNode = this.createReverbGraph_(reverbInputNode);
 ...
 */
   ...
   */
  this.setParameter = function(value, id) {
   ...
   */
   this.setReduction = function (value) {
   ...
   */
  this.setReverbWetness = function(value) {
   ...
   */
  this.setConvolverBuffer = function (impulseResponseBuffer) {
   ...
   */
   this.setDrumSample = function(drumSampleBuffer) {
    this.drumSource_ = new AudioBufferSourceNode(this.context_, {
    ...
    var drumDownMixer_ = new GainNode(
   ...
   */
  function setDrumSamplePlaybackRate(value) {
   ...
   */
  function setDrumSampleVolume(value) {
   ...
   */
  this.setNoisegateThreshold = function(value) {
   ...
   */
  this.setNoisegateAttack = function (value) {
   ...
   */
  this.setNoisegateRelease = function(value) {
   ...
   */
  this.setDelayWetness = function(value) {
   ...
   */
  this.setFeedbackDelayTime = function(value) {
   ...
   */
  this.setFeedbackDelayGain  = function(value) {
   ...
   */
  this.playVoice = function (noteName, frequency) {
    console.log('playing voice');
    var voice = new PolySynthVoice(this.context_, noteName, frequency, this);
    voice.output.connect(this.voiceOutput_);
    this.activeVoices_[noteName] = voice;
    voice.start();
  }
  
  this.rampNoteToFrequencyAtTime = function(noteName, frequency, time) {
      this.activeVoices_[noteName].setFrequency(frequency,time);
  }
   ...
   */
  this.releaseVoice = function(noteName) {
    // Move reference to voice to |this.releasedVoices_|.
    var voice = this.activeVoices_[noteName];
   ...
   */
  this.endVoice = function(noteName) {
